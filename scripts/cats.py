...
class Cat(object):
    ...
    def __init__(self, prefix=None, gender=None, status="kitten", parent1=None, parent2=None, pelt=None, eye_colour=None, 
    suffix=None, ID=None, moons=None, example=False, tortiecolour=None):
        self.gender = gender
        self.status = status
        self.age = None
                ...
                if cat.status == 'kitten':
                    ...
                    if cat.trait == 'bloodthirsty':
                        ...
                        thoughts.extend(['Is climbing to the top of a very tall tree!', 'Is racing through camp, accidentally knocking into clanmates',
                                         ...
                                         'Is challenging some warriors to a sparring match, one-on-one!', 'Is leaping from boulder to boulder out in the forest',
                        ...
                        thoughts.extend(['Is sniffing the edges of the camp wall suspiciously...', 'Smells like they may have rolled in catmint recently...',
                                         'Is currently eavesdropping on GOLDTWO clanmates, ears pricked', 'Is avoiding the conversation topic at paw',
                    ...
                    if cat.trait == 'bloodthirsty':
                        ...
                        thoughts.extend(['Is climbing to the top of a very tall tree!', 'Is racing through camp, accidentally knocking into clanmates',
                                         ...
                                         'Is challenging some clanmates to a sparring match, one-on-one!', 'Is leaping from boulder to boulder out in the forest',
                    ...
                    if cat.trait == 'bloodthirsty':
                        ...
                        thoughts.extend(['Accidentally assigns the same cat to GOLDTHREE patrols', 'Insists a hunting patrol only bring back mice',
    ...
    def update_sprite(self):
        ...
        if self.pelt is None:
            if self.parent1 is None:
                ...
                # setting the colour for torties/calis
        if self.pelt.name in ['Calico', 'Tortie']:
            if self.tortiecolour is None:
                if self.pelt.patch_colour in ['GOLDONE', 'GOLDTWO', 'GOLDTHREE', 'GOLDFOUR', 'GINGERONE', 'GINGERTWO', 'GINGERTHREE', 'GINGERFOUR']:
                        self.tortiecolour = choice(["BLACK", "DARKGREY", "BROWN", "DARKBROWN"])
                elif self.pelt.patch_colour in ['PALEONE', 'PALETWO', 'PALETHREE', 'PALEFOUR']:
                        self.tortiecolour = choice(["GREY", "PALEGREY", "SILVER", "LIGHTBROWN"])
                else:
                        self.tortiecolour = choice(["BLACK", "DARKBROWN"])
            return ''

            # pattern for tortie/calico cats
        if self.pelt.name in ['Calico', 'Tortie']:
                if self.pelt.tortiebase == "tabby":
                    self.pattern in 'tortietabby'
                elif self.pelt.tortiebase == "bengal":
                    self.pattern in 'tortiebengal'
                elif self.pelt.tortiebase == "marbled":
                    self.pattern in 'tortiemarbled'
                elif self.pelt.tortiebase == "ticked":
                    self.pattern in 'tortieticked'
                elif self.pelt.tortiebase == "rosette":
                    self.pattern in 'tortierosette'
                elif self.pelt.tortiebase == "smoke":
                    self.pattern in 'tortiesmoke'
                elif self.pelt.tortiebase == "speckled":
                    self.pattern in 'tortiespeckled'
                else:
                    self.pattern = 'tortiesolid'
        else:
            self.pelt.name == self.pelt.name
        ...
        if self.pelt.name not in ['Tortie', 'Calico']:
            ...
            if self.pelt.length == 'long' and self.status not in ['kitten', 'apprentice', 'medicine cat apprentice'] or self.age == 'elder':
                new_sprite.blit(sprites.sprites[self.pelt.tortiebase + 'extra' + self.tortiecolour + str(self.age_sprites[self.age])], (0, 0))
                game.switches['error_message'] = 'There was an error loading a tortie\'s pattern sprite. Last cat read was ' + str(self)
                new_sprite.blit(sprites.sprites[self.pelt.sprites[1] + 'extra' + self.patch_colour + str(self.age_sprites[self.age])], (0, 0))
            else:
                new_sprite.blit(sprites.sprites[self.pelt.tortiebase + self.tortiecolour + str(self.age_sprites[self.age])], (0, 0))
                game.switches['error_message'] = 'There was an error loading a tortie\'s pattern sprite. Last cat read was ' + str(self)
                new_sprite.blit(sprites.sprites[self.pelt.sprites[1] + self.patch_colour + str(self.age_sprites[self.age])], (0, 0))
        ...
        if game.settings['shaders'] and not self.dead:
            ...
            if self.pelt.length == 'long' and self.status not in ['kitten', 'apprentice', 'medicine cat apprentice'] or self.age == 'elder':
                new_sprite.blit(sprites.sprites['lineartdead' + str(self.age_sprites[self.age] + 9)], (0, 0))
            else:
                new_sprite.blit(sprites.sprites['lineartdead' + str(self.age_sprites[self.age])], (0, 0))
        ...
        if len(cat_data) > 0:
            ...
            for i in cat_data.split('\n'):
                ...
                if i.strip() != '':
                    ...
                    if len(attr) > 38:
                        the_cat.tortiecolour = attr[10]
                    if len(attr) > 45 and attr[45] is not None:                 #KEEP THIS AT THE END
                        the_cat.former_apprentices = attr[45].split(';')
