...
class Cat(object):
    ...
    suffix=None, ID=None, moons=None, example=False):
        self.gender = gender
        self.status = status
        self.age = None
    ...
    def update_sprite(self):
        ...
        if self.pelt.name in ['Calico', 'Tortie']:
            if self.pelt.colour in ['BLACK', 'DARKBROWN']:
                ...
                self.pattern = choice(['GOLDONE', 'GOLDTWO', 'GOLDTHREE', 'GOLDFOUR', 'GINGERONE', 'GINGERTWO', 'GINGERTHREE', 'GINGERFOUR'])
            elif self.pelt.colour in ['GREY', 'LIGHTBROWN', 'SILVER']:
                self.pattern = choice(['PALEONE', 'PALETWO', 'PALETHREE', 'PALEFOUR'])
        else:
            self.pattern = None
        ...
        if len(cat_data) > 0:
            ...
            for i in cat_data.split('\n'):
                ...
                if i.strip() != '':
                    ...
                    if len(attr) > 39:
                        the_cat.tortiebase = attr[39]
                    if len(attr) > 40:
                        the_cat.tortiepattern = attr[40]
    ...
    def describe_color(self):
        ...
        elif self.pelt.name in ["Tortie", "Calico"]:
            color_name = 'tortie'  # check for calico or for white later
            if self.pattern not in ["tortiesolid", "tortiesmoke"]:
                 color_name = 'torbie'
